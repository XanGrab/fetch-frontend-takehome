...
- [ ] Login form has no client side validation, and upon error presents no useful message to the user
- [ ] Some mechanism to not have logs display, at least by default, in the deployed version of the app
- [ ] Instead of making a singular fetch for all dog IDs needed, a call to /dogs per dog ID is being made, this may have been a decision for more compatibility with react query, but the additional overhead is almost certainly not worth the minor gain from react query in this case
- [ ] UI responsiveness could have been improved
...
- [ ] ~~Users should be able to modify this sort to be ascending or descending~~ (Issues with API, couldn't find the appropriate value for `sort` tried `field:[asc]`, `[asc]`, and `asc`)
